...
\options
* |namespace <on/off>| = specify whether survey creates namespace
  \defval{on}
...
\syntax
      |centreline [OPTIONS]
          ...
          extend <spec> [<station> [<station> [<station>]]]
...
\comopt
  ...
  * |team <person> [<roles>]| = a survey team member. The first argument
    ...
    \vtop{\hsize.19\hsize #} &
    \vtop{\hsize.19\hsize #} &
    \vtop{\hsize.60\hsize \strut#\strut} \cr
    {\it role}  & {\it alias}  & {\it intended meaning} \cr
    \noalign{\hrule}
      tape        & length       & Measured leg lengths \cr
      compass     & bearing      & Measured bearings \cr
      clino       & gradient     & Measured vertical angles \cr
      backtape    & backlength   & Like |tape| but for backsights \cr
      backcompass & backbearing  & Like |compass| but for backsights \cr
      backclino   & backgradient & Like |clino| but for backsights \cr
      instruments & insts        & All instruments: both compass and clino; use for
                                   all-in-one instruments such as DistoX. \cr
      counter     & count        & Topofil length measurements \cr
      depth       &              & Measured differences in height between stations,
                                   e.g.\ underwater with a diver's depth gauge, or above water with a manometer \cr
      station     &              & Added markers at stations \cr
      position    & \mgshift3.215\hsize\NEW{6.3.5}gps   & Recorded absolute positions of stations (e.g.\ fixed
                                   surface stations with a GPS) \cr
      notes       & notebook     & Recorded instrument readings \cr
      pictures    & pics         & Drew sketches \cr
      assistant   & dog          & General helper (e.g.\ held the end of the tape on stations) \cr
      altitude    & dz           & Recorded the altitudes of stations (e.g.\ with an altimeter) \cr
      dimensions  &              & Measured all passage dimensions \cr
      left        &              & Measured |left| passage dimension \cr
      right       &              & Measured |right| passage dimension \cr
      up          & ceiling      & Measured |up| passage dimension \cr
      down        & floor        & Measured |down| passage dimension \cr
      explorer    &              & Explored the area of cave being surveyed \cr
    }
  ...
  * |break| = can be used with interleaved data to separate two traverses.
    It also prevents consecutive shots between the same stations from
    being averaged as repeat measurements
  ...
  * |flags <shot flags>| = set flags for following shots. The supported
    ...
    in maps and models by default) and \NEW{6.4.1}|artificial| (for
    man-made passages, such as dug or blasted connections). These are
    excluded from length calculations.

    |artificial| shots are drawn and modelled like any other shot, and
    they take part in loop closure. Only the length statistics ignore
    them. A shot that already carries |surface|, |duplicate| or |splay|
    keeps being counted with that flag; adding |artificial| to it changes
    nothing.
  ...
  * |data <style> <readings order>| = set data style (normal, topofil,
    ...
    If consecutive data lines specify the same from and to stations,
    they are treated as repeat measurements of a single leg and are
    automatically averaged.
    Only consecutive lines are averaged; use |break| to prevent
    averaging between groups of data.
...
\NEW{5.3}    If one of the shot stations is named either `|.|' or `|-|', the shot has
  ...
  * |extend <spec> [<station> [<station> [<station>]]]| =
    control how the centerline is extended. |<spec>| may be one of the
    following keywords

    |normal/reverse| = extend given and following stations
     to the same/reverse direction as previous station. If station names are
     given, direction is applied only to the specified station or shot.
    ...
    or a percentage in the range |0| to |200|. This sets the extend ratio,
    i.e.\ the factor used for the horizontal component of a leg in extended
    elevation. Therion keeps the vertical component unchanged and multiplies
    only the horizontal displacement by the ratio. Thus |100| means the normal
    width of the extended leg, |50| compresses it to one half, |200| stretches
    it to double width, and |0| suppresses any horizontal displacement
    (effectively the same result as |vertical| for that leg).

    The optional station arguments are interpreted as follows:

    |extend <spec>| = set the default extend behaviour for subsequent shots
    in the current centreline.

    |extend <spec> <from>| = apply the extend setting to a station.

    |extend <spec> <from> <to>| = apply the extend setting to a specific shot.

    |extend <spec> <before> <from> <to>| = apply the extend setting to a
    specific shot, with an additional |before| station condition.
    This form is primarily useful with |ignore|, where the shot is ignored only
    if the extended-elevation traversal reaches |<from>| from |<before>|.

    Station names use the normal Therion object reference syntax, so
    |station@survey| may be used.
...
\options
  ...
  * |flip none/horizontal/vertical| = flips the scrap after
    scale transformation \defval{none}.
  * |station-names <prefix> <suffix>| = adds given prefix/suffix to
...
\options
  ...
  * |scale| = symbol scale, can be:
    ...
    numeric value \defval{normal}. Named sizes scale by $\sqrt 2$, so that
    $xs \equiv 0.5$, $s \equiv 0.707$, $m \equiv 1.0$, $l \equiv 1.414$ and
    $xl \equiv 2.0$.
  * |place <bottom/default/top>| = changes displaying order in the map \defval{default}.
  * |clip <on/off>| = specify whether a symbol is clipped by the scrap border.
    ...
    \defval{on}.
  * |visibility <on/off>| = displays/hides the symbol \defval{on}.
...
\NEW{5.3}|<lang:XX>| = creates multilingual label (see |string| type for
  ...
  * |value| = value of height, passage-height, altitude, dimensions or date
      ...
      `|+?|' and `|-?|' are also accepted, meaning unknown height or depth.
...
\comopt
    ...
    As a line point option (inside [LINE DATA]) it can be used multiple times.
       ...
       * |smooth <on/off/auto>| = whether the line is smooth at the given point.
         Deafult is |auto|. This setting only has any effect if both segments that
       ...
       * |adjust <horizontal/vertical>| = shifts the line point to be aligned
         ...
         first point. The result is a horizontal/vertical line segment. When unset
         (the default situation) the line point isn't automatically aligned with the
         previous line point. This option is not allowed in the |plan| projection.
       * |place <bottom/default/top>| = changes displaying order in the map. Default
         is |top| for |ceiling-step| and |chimney|, |default| for all other types.
       * |clip <on/off>| = specify whether a symbol is clipped by the scrap border
         \defval{on}.
       * |visibility <on/off>| = displays/hides the symbol \defval{on}.
       * |context <point/line/area> <symbol-type>| = (to be used with |symbol-hide|
   ...
   (which is 0 by default).
       ...
       * |direction <begin/end/both/none/point>| = can be used only
         ...
         a direction arrow on the section line \defval{none}. The |point|
       ...
       * |gradient <none/center/point>| = can be used only with the contour
         ...
         option must be used inside |[LINE DATA]|. The others can (and should)
         be used as a line option \defval{none}.
       * |head <begin/end/both/none>| = can be used only with the arrow
         type and indicates where to put an arrow head \defval{end}.
...
the same line as the |area| command. The last one appearing prevails.
  ...
  * |place <bottom/default/top>| = changes displaying order in the map \defval{top}.
  * |clip <on/off>| = specify whether a symbol is clipped by the scrap border \defval{on}.
  * |visibility <on/off>| = displays/hides the symbol \defval{on}.
  * |context <point/line/area> <symbol-type>| = (to be used with |symbol-hide|
    and |symbol-show| layout options) symbol will be hidden/shown according
    to rules for specified |<symbol-type>|.
...
\description
 The |scan| command imports a 3D triangulated mesh into the cave model. It is
 intended for meshes measured outside Therion, for example by laser scanning or
 photogrammetry. The mesh is tied to the survey centreline by calibration lines:
 each calibration line gives the coordinates, in the mesh coordinate system, of
 one survey station already known from the centreline.

 The command contributes geometry to 3D model exports. It does not create 2D map
 symbols and does not replace scraps used for map drawing.
\enddescription
...
\arguments
 The |scan| command itself has no obligatory arguments. Data lines inside the
 command have the following form:

 * |<station>| = name of an existing survey station. If the |scan| command is
  inside a |survey|, normal survey name-space rules apply. Use |station@survey|
  if a station from another visible survey must be referenced.

 * |<x> <y> <z>| = coordinates of |<station>| in the mesh coordinate system.
  These numbers are interpreted using the current |units| command-like option
  of the |scan| object.
\endarguments

\comopt

 * |file <file-name>| = mesh file to import. This command-like option is
  required. The current implementation reads STL meshes. Relative file names are
  resolved relative to the file that contains the |scan| command.

 * |units <length units>| = length unit used by coordinates in the mesh file and
  in the station calibration lines \,\defval{m}.

 * |axes <x-axis> <y-axis> <z-axis>| = orientation of the mesh coordinates before
  the station calibration is applied. Each argument is one of |x|, |y|, |z|,
  |-x|, |-y| or |-z|, and the three arguments must contain all three coordinate
  names. The default is |x y z|. The first argument defines Therion's local
  |x| coordinate, the second its local |y| coordinate, and the third its local
  |z| coordinate. For example, |axes -z -x y| transforms a mesh point
  |(x,y,z)| to |(-z,-x,y)|.
...
\notes
 ...
 Calibration lines should refer to stations whose 3D positions are already known
 from the centreline. A single station can only shift the mesh. With two stations
 Therion can determine horizontal rotation and scale; additional well-distributed
 stations give a better local morphing of the mesh. Avoid using stations that are
 very close to each other or nearly vertical above one another as the only
 calibration points.

 The |units| and |axes| command-like options are applied both to vertices read
 from the mesh file and to the station coordinates given in the |scan| block.
 If an imported mesh is mirrored or rotated, change |axes| rather than changing
 the station calibration coordinates.
...
scrap may then contain arbitrary parts of any survey in the last level of
